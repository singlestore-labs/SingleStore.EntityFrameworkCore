...
{
    // TODO: Revamp
    public class SingleStoreUpdateSqlGenerator : UpdateSqlGenerator, ISingleStoreUpdateSqlGenerator
    {
        /// <summary>
            ...
            int commandPosition)
        {
            ...
            if (modificationCommands.Count == 1
                && modificationCommands[0].ColumnModifications.All(o =>
                    !o.IsKey
                    || !o.IsRead
                    || o.Property?.GetValueGenerationStrategy(table) == SingleStoreValueGenerationStrategy.IdentityColumn))
            {
            ...
            var nonIdentityOperations = modificationCommands[0].ColumnModifications
                .Where(o => o.Property?.GetValueGenerationStrategy(table) != SingleStoreValueGenerationStrategy.IdentityColumn)
                .ToList();
            ...
            IReadOnlyList<IColumnModification> operations)
        {
            ...
            // Because we currently use `SingleStoreValueGenerationStrategy.IdentityColumn` for auto_increment columns as well
            // as CURRENT_TIMESTAMP columns, we need to use `SingleStorePropertyExtensions.IsCompatibleAutoIncrementColumn()`
            // to ensure, that the column is actually an auto_increment column.
            // See https://github.com/PomeloFoundation/EntityFrameworkCore.SingleStore/issues/1300
            var nonDefaultOperations = operations
                .Where(
                    o => !o.IsKey ||
                         ...
                         SingleStorePropertyExtensions.IsCompatibleAutoIncrementColumn(o.Property))
                .ToList()
                .AsReadOnly();
