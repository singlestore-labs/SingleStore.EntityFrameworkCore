...
LEFT JOIN (
    ...
    LEFT JOIN (
        ...
        FROM (
            SELECT {(AppConfig.ServerVersion.Supports.SingleStoreBug96947Workaround ? "CAST(1 AS signed)" : "1")} AS `c`, `l1`.`Id`, `l1`.`OneToMany_Optional_Inverse3Id`, ROW_NUMBER() OVER(PARTITION BY `l1`.`OneToMany_Optional_Inverse3Id` ORDER BY `l1`.`Id`) AS `row`
            FROM `LevelThree` AS `l1`
        ) AS `t`
        WHERE `t`.`row` <= 1
