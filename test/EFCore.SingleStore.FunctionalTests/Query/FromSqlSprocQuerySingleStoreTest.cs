...
{
    public class FromSqlSprocQuerySingleStoreTest : FromSqlSprocQueryTestBase<NorthwindQuerySingleStoreFixture<NoopModelCustomizer>>
    {
        public FromSqlSprocQuerySingleStoreTest(
            NorthwindQuerySingleStoreFixture<NoopModelCustomizer> fixture, ITestOutputHelper testOutputHelper)
            : base(fixture)
        {
        ...
        public override async Task From_sql_queryable_stored_procedure_projection(bool async)
        {
            ...
            if (async)
            {
                await Assert.ThrowsAsync<SingleStoreException>(() => query.ToArrayAsync());
            }
            else
            {
                Assert.Throws<SingleStoreException>(() => query.ToArray());
            }
        }
        ...
        public override async Task From_sql_queryable_stored_procedure_min(bool async)
        {
            ...
            if (async)
            {
                await Assert.ThrowsAsync<SingleStoreException>(() => query.MinAsync(mep => mep.UnitPrice));
            }
            else
            {
                Assert.Throws<SingleStoreException>(() => query.Min(mep => mep.UnitPrice));
            }
        }
